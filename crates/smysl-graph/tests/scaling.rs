...
/// `pack` with a budget that **binds**, which is the case that is still super-linear.
///
/// With an ample budget 0.3.0 skips the greedy entirely — if the whole scope fits at its top
/// level there is nothing to trade — so that path is linear and uninteresting. The cost
/// lives where the budget forces a choice: the greedy runs one round per unit admitted and
/// re-evaluates every remaining candidate each round, which is O(n²) by construction rather
/// than by accident.
///
/// This is the baseline any fix has to beat. Kept next to the `salience` measurement so the
/// two are read together: one operation was assumed linear and is, the other was assumed
/// linear and is not.
#[test]
#[ignore = "a measurement, not a gate"]
