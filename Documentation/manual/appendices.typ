...
#dtable(
  ...
  (
    ...
    ([`--source`], [`PREFIX`], [Pack only units whose source reference starts with `PREFIX`: the whole of one subject out of a shared store. Unlike `find --source` this scopes the *pack*, not the query.]),
...
#dtable(
  ...
  (
    ...
    ([`--source`], [`PREFIX`], [Restrict to units whose source reference starts with `PREFIX` — one subject's units out of a store holding many. A prefix nothing matches says so rather than searching everything.]),
