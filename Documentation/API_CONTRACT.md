...
- **The facade's 254 names at `--all-features`, 209 at `--no-default-features`.** None moves
  ...
  provider layer; `ingest` implies it, so no name left `ingest`. 1.4 adds `Withdrawal`,
  `Resolution`, `ResolutionTarget`, `review`, `ReviewItem` and `ReviewSubject` (260 and 215 so
  far in the cycle).
