...
fn the_manual_still_describes_this_binary() {
    ...
    // 1.2 replayed chapters as sequences and reached 85. Set just under the real number: high
    ...
    // a chapter, a label mistaken for a missing file — so the gap between 80 and 85 is not
    // where the risk lives.
    assert!(
        ran >= 80,
        "the replay covered {ran} transcripts, which is too few to mean anything — 85 of the \
